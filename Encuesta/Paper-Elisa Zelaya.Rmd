...
#### Resumen estadistico previo a la trasformación
Se identificaron valores vacios y se realizo la umputacion de crear una categoriallamada No aplica, para la el porcentaje de la poblacion que no aplicada en la respuesta a esta variable..

```{r transformacion2, echo=FALSE}


survey <- survey %>%
  mutate(factores_no_cambio = as.factor(ifelse(((survey$factores_no_cambio == '' )
                                                & (survey$pensado_cambio_carrera == 'Sí' | survey$pensado_cambio_carrera =='Tal vez')), 'No contestó', as.character(factores_no_cambio))))

survey <- survey %>%
  mutate(factores_no_cambio = as.factor(ifelse(((survey$factores_no_cambio == '')  & (survey$pensado_cambio_carrera == 'No')), 'No aplica', as.character(factores_no_cambio))))

survey <- survey %>%
  mutate(factores_no_cambio = as.factor(ifelse(((survey$factores_no_cambio == '')  & (survey$cambio_carrera == 'Sí')), 'No aplica', as.character(factores_no_cambio))))
